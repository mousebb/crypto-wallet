...
class TronWallet {
  ...
  Future<Uint8List> _getTransactionBytes(Map<String, dynamic> transaction) async {
    if (transaction == null || transaction['txID'] == null) {
      throw Exception('获取交易字节失败: 交易ID为空');
    }
    
    print('获取交易字节请求:');
    print('URL: $nodeUrl/wallet/gettransactionbyid');
    print('请求数据: ${json.encode({
      'value': transaction['txID'],
      'visible': true
    })}');
    
    final response = await http.post(
      Uri.parse('$nodeUrl/wallet/gettransactionbyid'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({
        'value': transaction['txID'],
        'visible': true
    ...
    if (response.statusCode != 200) {
      throw Exception('获取交易字节失败: ${response.body}');
    }
    
    final result = json.decode(response.body);
    if (result == null) {
      throw Exception('获取交易字节失败: 返回数据为空');
    }
    
    // 检查原始数据字段
    if (result['raw_data'] == null) {
      throw Exception('获取交易字节失败: 缺少raw_data字段');
    }
    
    // 获取原始数据的十六进制表示
    final rawDataHex = result['raw_data_hex'] ?? result['raw_data']['data'];
    if (rawDataHex == null) {
      throw Exception('获取交易字节失败: 缺少raw_data_hex字段');
    }
    
    try {
      return Uint8List.fromList(HEX.decode(rawDataHex));
    } catch (e) {
      throw Exception('获取交易字节失败: 解析原始数据失败 - $e');
    }
  }
  ...
  static String generatePrivateKeyFromMnemonic(String mnemonic) {
    ...
    return HEX.encode(seed.sublist(0, 32));
  }
  
  static Map<String, String> generateNewWallet() {
    final 
  mnemonic = bip39.generateMnemonic();
    final privateKey = generatePrivateKeyFromMnemonic(mnemonic);
    return {
      'mnemonic': mnemonic,
...
void main() async {
  // 示例用法
  // final walletData = TronWallet.generateNewWallet();
  // print('新钱包生成:');
  // print('助记词: ${walletData['mnemonic']}');
  // print('私钥: ${walletData['privateKey']}');
  ...
  try {
    // TRX转账示例
    final txId = await wallet.sendTRX('TYS5AXgfeeKau1MTz7UcVfyXoQ5bZ5cybV', 1000000);
    print('TRX转账成功，交易ID: $txId');
    
    // TRC20转账示例
    // final trc20TxId = await wallet.sendTRC20(
    //   TronWallet.usdtTestnet,
    //   '接收地址',
    //   BigInt.from(1000000), // 1 USDT (6 decimals)
    // );
    // print('TRC20转账成功，交易ID: $trc20TxId');
  } catch (e) {
    print('转账失败: $e');
  }
